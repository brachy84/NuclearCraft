	...
	public static class Handler extends TileGuiPacket.Handler<OpenSideConfigGuiPacket> {
		...
		protected void onPacket(OpenSideConfigGuiPacket message, EntityPlayerMP player, TileEntity tile) {
			if (tile instanceof ITileGui<?, ?, ?> tileGui) {
				FMLNetworkHandler.openGui(player, NuclearCraft.instance, tileGui.getContainerInfo().guiId + 1000, player.getServerWorld(), message.pos.getX(), message.pos.getY(), message.pos.getZ());
